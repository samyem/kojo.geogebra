...
public class Kernel {
	...
	public static final int COORD_STYLE_FRENCH = 2; // A: (3, 2) and B: (3;
													// 90�)
	...
	final public static char EULER_CHAR = '\u212f'; // "\u0435";
	...
	private String casPrintFormPI; // for pi and degree sign
	...
	// /**
	// * Defines all functions part of vars in the GeoGebraCAS.
	// * Note: if the CAS is not loaded yet, nothing is done.
	// */
	// final public void defineFunctionsInCAS(Set vars) {
	...
	// String mathPiperString = fun.getLabel() + "(x) := " +
	// fun.getFunctionExpression().getCASstring(ExpressionNode.STRING_TYPE_MATH_PIPER,
	// true);
	// evaluateMathPiper(mathPiperString);
	//				
	// System.out.println("SET: " + mathPiperString);
	// break;
	// }
	// }
	// }
	//	
	// /**
	// * Removes all function definitions part of vars from the GeoGebraCAS.
	// * Note: if the CAS is not loaded yet, nothing is done.
	// */
	// final public void removeFunctionsFromCAS(Set vars) {
	// if (ggbCAS == null) return;
	//		
	// Iterator it = vars.iterator();
	// while (it.hasNext()) {
	...
	// case GeoElement.GEO_CLASS_FUNCTION:
	// GeoFunction fun = (GeoFunction) geo;
	// String mathPiperString = "Retract(" + fun.getLabel() + ", 1)";
	// evaluateMathPiper(mathPiperString);
	//					
	// System.out.println("REMOVE: " + mathPiperString);
	// break;
	// }
	// }
	// }
	...
	final public void restorePrintAccuracy() {
		...
		useSignificantFigures = ((Boolean) useSignificantFiguresList.pop())
				.booleanValue();
		int sigFigures = ((Integer) (noOfSignificantFiguresList.pop()))
				.intValue();
	 ...
	 */
	final public static GeoElement createGeoElement(Construction cons,
			String type) throws MyError {
	...
	public void setNotifyViewsActive(boolean flag) {
		...
		if (flag != notifyViewsActive) {
			...
			if (flag) {
				...
				/*
				 * Object [] geos =
				 * getConstruction().getGeoSetConstructionOrder().toArray(); for
				 * (int i = 0 ; i < geos.length ; i++) { GeoElement geo =
				 * (GeoElement) geos[i]; notifyAdd(geo); }
				 */
	 ...
	 * @return
	 */
	final public GeoList List(String label, ArrayList geoElementList,
	...
	final public GeoLine DependentLineAdd(GeoLine line1, GeoLine line2,
			ExpressionNode root) {
		AlgoDependentLine algo = new AlgoDependentLine(cons, line1, line2,
				AlgoDependentLine.MODE_ADD, root);
		GeoLine line = algo.getLine();
		return line;
	}

	final public GeoLine DependentLineSubtract(GeoLine line1, GeoLine line2,
			ExpressionNode root) {
		AlgoDependentLine algo = new AlgoDependentLine(cons, line1, line2,
				AlgoDependentLine.MODE_SUBTRACT, root);
		GeoLine line = algo.getLine();
		return line;
	}

	final public GeoLine DependentLineMultiply(GeoLine line1, NumberValue num,
			ExpressionNode root) {
		AlgoDependentLine algo = new AlgoDependentLine(cons, line1, num,
				AlgoDependentLine.MODE_MULTIPLY, root);
		GeoLine line = algo.getLine();
		return line;
	}

	final public GeoLine DependentLineDivide(GeoLine line1, NumberValue num,
			ExpressionNode root) {
		AlgoDependentLine algo = new AlgoDependentLine(cons, line1, num,
				AlgoDependentLine.MODE_DIVIDE, root);
		GeoLine line = algo.getLine();
		return line;
	}
	...
	final public GeoNumeric Distance(String label, GeoPoint P, GeoLine g) {
		AlgoDistancePointLine algo = new AlgoDistancePointLine(cons, label, P,
				g);
		GeoNumeric num = algo.getDistance();
		return num;
	}
	...
	final public GeoElement[] Rotate(String label, Rotateable geoRot,
		...
		if (geoRot.toGeoElement().isLimitedPath())
			...
			return ((LimitedPath) geoRot).createTransformedObject(
					TRANSFORM_ROTATE, label, null, null, null, phi);
	...
	final public GeoElement[] Dilate(String label, Dilateable geoRot,
		...
		if (geoRot.toGeoElement().isLimitedPath())
			// handle segments, rays and arcs separately
			return ((LimitedPath) geoRot).createTransformedObject(
					TRANSFORM_DILATE, label, S, null, null, r);
	 ...
	 */
	final public GeoElement[] Translate(String label, GeoPolygon poly,
			GeoVector v) {
	...
	final public GeoElement[] Rotate(String label, GeoPolygon poly,
		...
		return transformPoly(label, poly,
				rotPoints(poly.getPoints(), phi, null));
	}
	...
	final public GeoElement[] Mirror(String label, GeoPolygon poly, GeoPoint Q) {
		return transformPoly(label, poly, mirrorPoints(poly.getPoints(), Q,
				null));
	}
	...
	final public GeoElement[] Mirror(String label, GeoPolygon poly, GeoLine g) {
		return transformPoly(label, poly, mirrorPoints(poly.getPoints(), null,
				g));
	}
	...
	private GeoElement[] transformPoly(String label, GeoPolygon oldPoly,
		...
		GeoElement ret[] = Polygon(polyLabel, transformedPoints);
		ret[0].setVisualStyleForTransformations(oldPoly);
		ret[0].update();
		return ret;
	}
	...
	public static boolean keepOrientationForTransformation(
	...
	final private StringBuffer buildImplicitVarPart(double[] numbers,
		...
		if (sbBuildImplicitVarPart == null)
			sbBuildImplicitVarPart = new StringBuffer(80);
	...
	private StringBuffer sbBuildImplicitVarPart;

	final StringBuffer buildImplicitEquation(double[] numbers, String[] vars,
			boolean KEEP_LEADING_SIGN, boolean CANCEL_DOWN) {

		if (sbBuildImplicitEquation == null)
			sbBuildImplicitEquation = new StringBuffer(80);
	...
	private StringBuffer sbBuildImplicitEquation;
