 ...
 * 			17.11:	sorting list, noisekiller (nearmaxmin() ),
 * 			19.11:  some more testing, small fixes
 ...
 *      The Levenberg-Marquardt method makes c and d significantly less critical.
 *  This led me to this algorithm:
 * 		I 	a=average of y-data
 * 		  	b=(maxy-miny)/2
 * 		II 	period=2*|x_first_max - x_first_min|
 *          c=2pi/period
 *          The first two extremums are found by my "direction-changing-algorithm" (Ulven nov-08), using a flank
 *          of three points as an indicator of monotonous increasing or decreasing function.
 *          (Same effort as finding a local max as y1<y2>y3, but effectively equivalent to using 5 points: y1<y2<y3>y4>y5)
 *		III simple iteration of d in <-pi,pi> to find a good d
 *			(Critical if c is a bit off, so better than pi/2-c*xmax)
 ...
 *      (Perhaps Donald Knuth could have done all this in 50 lines, but this is the best I can do...)
 * 	Constraints:
 * 		<List of points> should have at least 5 points. 
 * 		There should also be three points on the row steadily increasing
 * 		or decreasing(y1<=y2<=y3 or y1>=y2>=y3)on each side/flank of the first extremums.
 * 		The points should cover at least two extremums of the function.
 * 		The two first extremums should not be too far from the extremums of the solution curve.
 * 		If more than one period, there should, at the very least, be more than 6 points in each period.
 * 		If any of these demands are not satisfied, the solution curve might be unusable.
 * 	Problems:
 ...
 * 		It might also diverge, so MAX_ITERATIONS is set to 200. (Experience suggests that more than 100 iterations give useless results.)
 *  ToDo:
 *  	Use Discrete Fourier Transform instead of step II and III. 	Experiments show that this adds surprisingly little to 
 *      the robustness of my simple algorithm, so this will not be done unless user feedback indicates a need for more sophistication.
 */
...
public class AlgoFitSin extends AlgoElement{
	...
	private final static double NOISEKILLER		=	0.2D;	//Kill local extremums inside a+/-noisekiller*b; 0.2 seems to be a good value?
...
/// ============= IMPLEMENTATION =============================================================///
    ...
    public final static void findParameters() {
        double 	y;
        double 	min_max_distance;		//Distance between x-values of found(?) extrema
        ...
        int xmax=xmax_abs,xmin=xmin_abs;	//Keep absolute xmax/xmin in case changes=0 or 1 later      
    ...
    public final static void sinus_Reg(){
        ...
        double 	da			=	EPSILON,
        		db			=	EPSILON,
        		dc			=	EPSILON,
        		dd			=	EPSILON;     //Something larger than eps, to get started...
        ...
        // Not wanted in log: System.out.println("AlgoFitSin: Sum Errors Squared= "+beta2(xd,yd,a,b,c,d));	//Info
    ...
    //that is done in findParameters() which is better for testing only mathematical functionality.)
    private final   void getPoints(){
        ...
        xd=xlist;yd=ylist;
        if(error){errorMsg("getPoints(): Wrong list format, must be points.");}
    }//getPoints()    
    ...
    private final static boolean nearmaxmin(double y,double a,double b,int state, int current,double max,double min){
    	if( (state==1) && (current==-1) ){  //A real max-change?
    		if(max>a+NOISEKILLER*b){return true;} else{return false;}	
    	}else if ( (state==-1) && (current==1) ){  //A real min-change?
    		if(min<a-NOISEKILLER*b){return true;} else {return false;}
    	}else{
    ...
    // Finds the number of halfperiods between abs max and abs min that gives the least SSE
...
/*    //SNIP START---------------------------------
    ...
    ///// ----- Test Interface ----- /////
    //Test implementation from outside Ggb
    //with datapoints in x[] and y[]
    public static boolean runTest(double[] x,double[] y){
    	//compute() did not run, have to make a a separate RegressionMath:
        regMath = new RegressionMath();	//RegressionMath should be cleaned up with only static!
    	xd=x;yd=y;
    	findParameters();
    	sinus_Reg();
    	return error;
    }//runtTest(x,y,a,b,c,sse,i)
    
    //Return the latest calculations:
    ...
    public static double getSSE(){return beta2(xd,yd,a,b,c,d);}
