...
//			return FunctionVariable.create("log",den).mult(quot);
		...
		// Markus Hohenwarter 2009-03-03 BEGIN
		// try polynomial division:
		// nom = q0 * denom + q1, thus 
		// Int(nom/denom) = Int((q0 * denom + q1) / denom) = Int(q0) + Int(q1/denom)
		Algebraic q[] = nom.div(den,null);
		// we only do this if q0 is not a fraction itself
		if(!(q[0] instanceof Rational) && !q[0].equals(Zahl.ZERO)) {					
			return q[0].integrate(var).add(q[1].div(den).integrate(var));
		}
//		if(!q[0].equals(Zahl.ZERO)) {					
//			return q[0].integrate(var).add(q[1].div(den).integrate(var));
//		}
		// Markus Hohenwarter 2009-03-03 END
